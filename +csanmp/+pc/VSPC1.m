function [nodes,solution,accepted,rejected,fval] = VSPC1(ode,domain,...
...
arguments (Input)
    ...
    tol (1,1) {mustBeReal,mustBePositive}
    step (1,1) {mustBeReal,mustBePositive} = 0.01*tol
...
arguments (Output)
    ...
    rejected (1,1) {mustBeInteger,mustBePositive}
    fval (1,1) {mustBeInteger,mustBePositive}
end

import csanmp.pc.milne

accepted = 0; rejected = 0; fval = 0;
n = 1; % Nodes counter
nodes(1) = domain(1);
